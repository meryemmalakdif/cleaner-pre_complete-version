...
class MNIST_truncated(data.Dataset):
    ...
    #     root: Root directory where the MNIST dataset is stored.
    #     dataidxs: (Optional) An array-like object containing indices specifying which data points to include from the original MNIST dataset.
    #     train: (Optional) Boolean flag indicating whether to use the training or testing set of the MNIST dataset (defaults to True for training).
    #     transform: (Optional) A function used to pre-process the image data (e.g., normalization, resizing).
    #     target_transform: (Optional) A function used to pre-process the target labels (e.g., one-hot encoding).
    #     download: (Optional) Boolean flag indicating whether to download the MNIST dataset if it's not already present.
    ...
    # this means from the original dataset we truncate only samples corresponding to specific indices
    def __build_truncated_dataset__(self):
        ...
        data_as_numpy = mnist_dataobj.data.numpy()

        # Check the shape of the data
        data_shape = np.shape(data_as_numpy)
...
# grayscale images of 10 fashion items
...
# Each image in CelebA is associated with 40 binary labels indicating the presence or absence of specific facial attributes. These attributes describe various aspects of the person's appearance, such as hair, eye color
...
# images dataset with 10 classes : plane, car ...
